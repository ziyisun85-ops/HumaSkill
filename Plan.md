...
**Validation commands:**
```bash
python scripts/validate_hermite.py
...
- `scripts/validate_matcher.py` — extend to test scoring accuracy against real trajectory states extracted from `result.json` outputs
...
**Acceptance criteria:**
- `python scripts/validate_matcher.py` passes with all assertions
